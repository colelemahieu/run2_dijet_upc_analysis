# this produces the Forest files, run with CMSSW_13_2_2
...
process.source = cms.Source("PoolSource",
    ...
    fileNames = cms.untracked.vstring(
        "/store/hidata/HIRun2018A/HIForward/MINIAOD/PbPb18_MiniAODv1-v1/100000/00c99cd7-e08c-4af7-b3cf-509adcb71968.root"
...
if addR2Jets or addR3Jets or addR3FlowJets or addR4Jets or addR4FlowJets or addR6Jets or addR8Jets :
    ...
    if addR2Jets :
        ...
        setupPprefJets('akCs2PF', process.jetsR2, process, isMC = 0, radius = 0.20, JECTag = 'AK2PF')
        process.akCs2PFpatJetCorrFactors.levels = ['L2Relative', 'L2L3Residual']
        process.akCs2PFJetAnalyzer = process.akCs2PFJetAnalyzer.clone(jetTag = "akCs2PFpatJets", jetName = 'akCs2PF')
        process.forest += process.extraJetsData * process.jetsR2 * process.akCs2PFJetAnalyzer    

    if addR6Jets :
        process.jetsR6 = cms.Sequence()
        setupPprefJets('akCs6PF', process.jetsR6, process, isMC = 0, radius = 0.60, JECTag = 'AK6PF')
        process.akCs6PFpatJetCorrFactors.levels = ['L2Relative', 'L2L3Residual']
        process.akCs6PFJetAnalyzer = process.akCs6PFJetAnalyzer.clone(jetTag = "akCs6PFpatJets", jetName = 'akCs6PF')
        process.forest += process.extraJetsData * process.jetsR6 * process.akCs6PFJetAnalyzer
	
    if addR8Jets :
        process.jetsR8 = cms.Sequence()
        setupPprefJets('akCs8PF', process.jetsR8, process, isMC = 0, radius = 0.80, JECTag = 'AK8PF')
        process.akCs8PFpatJetCorrFactors.levels = ['L2Relative', 'L2L3Residual']
        process.akCs8PFJetAnalyzer = process.akCs8PFJetAnalyzer.clone(jetTag = "akCs8PFpatJets", jetName = 'akCs8PF')
        process.forest += process.extraJetsData * process.jetsR8 * process.akCs8PFJetAnalyzer
    ...
    if addR4Jets :
        ...
        setupPprefJets('akCs0PF', process.jetsR4, process, isMC = 0, radius = 0.40, JECTag = 'AK4PF')
        ...
        process.akFlowPuCs4PFJetAnalyzer.matchTag = 'ak4PFMatchingFor' + 'akCs0PF' + 'patJets'
...
#process.primaryVertexFilter.InputTag = "offlineSlimmedPrimaryVerticesRecovery"
