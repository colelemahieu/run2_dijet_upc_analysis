// getData_histograms_0n0n.C
// this code calculates the v2=<cos(2phi)> variable
// this code also applies final event selections to 0n0n data and writes histograms to a file
...
void getData_histograms_0n0n()
{
  bool thirdJetCut=true;
  bool dPhiCut=false;
  
  // read 0n0n data
  TFile *file = new TFile("../miniAOD_r4_0n0n.root");  
  ...
  evtTree->SetBranchAddress("lumis", &lumis);
  ...
  TH1F *h_raps = new TH1F("h_raps","h_raps",30,-2.5,2.5);
  ...
  // 2D Phi vs Rapidity
  const int nBins=5;
  double rapidityBins[nBins+1]= {-2.4,0.0,0.5,1.0,1.5,2.4};
  ...
  TH2D *h2_phiQTPT_QT_mpi_24 = new TH2D("h2_phiQTPT_QT_mpi_24","h2_phiQTPT_QT_mpi_24",5,0,40,24,-TMath::Pi(),TMath::Pi());
  ...
  float pfId_chH_sum=0, pfId_chH_sum_less300=0;
  ...
  for (int i=0; i<jetEnt; i++)
    {
      ...
      eeMax_0nXn=0;
      eeMax_Xn0n=0;
      pfId_chH_sum=0, pfId_chH_sum_less300=0;
      ...
      if (ZDC_plus<1200)
	{
      ...
      //if (angle12>=0) angle12 = angle12;
      //if (angle12<0) angle12 = angle12 + 2*pi;
      ...
      for (int iPF=0; iPF<nPF; iPF++)
	{
      ...
      // apply noise and rapidity gap cuts
      if (dijetRap>0)
	{
	  ...
	  if (fabs(backward_rapgap)<trackGap) continue;
	  if (fabs(backward_rapgap)<forward_rapgap) continue;
	}
      if (dijetRap<0)
	{
	  ...
	  if (fabs(forward_rapgap)<backward_rapgap) continue;
	}

       // georgios test
      float leadingPt=0, leadingEta=0, leadingPhi=0;
      for (long unsigned int iTrk=0; iTrk<pfTrkPt->size(); iTrk++)
	{
	  if (pfTrkPt->at(iTrk)==-99) continue;
	  if (pfTrkPt->at(iTrk)<0.3 || pfTrkPt->at(iTrk)>20) continue;
	  if (pfTrkEta->at(iTrk)>(jetEta[0]+jetEta[1])/2) continue;
	  if (fabs(pfTrkEta->at(iTrk))>2.5) continue;

	  if (pfTrkPt->at(iTrk)>leadingPt)
	    {
	      leadingPt=pfTrkPt->at(iTrk);
	      leadingEta=pfTrkEta->at(iTrk);
	      leadingPhi=pfPhi->at(iTrk);
	    }
	   
	}
      float dist0 = sqrt((jetEta[0]-leadingEta)*(jetEta[0]-leadingEta) + (jetPhi[0]-leadingPhi)*(jetPhi[0]-leadingPhi));
      float dist1 = sqrt((jetEta[1]-leadingEta)*(jetEta[1]-leadingEta) + (jetPhi[1]-leadingPhi)*(jetPhi[1]-leadingPhi));
      float deltaEta = ((jetEta[0]+jetEta[1])/2) - leadingEta;
      ...
      if (rapPrime>1.5 && rapPrime<2.4)
	{
      ...
      h_dijetRap->Fill((jet1+jet2).Rapidity());
      h_dijetRap_abs->Fill(fabs((jet1+jet2).Rapidity()));
      h_raps->Fill(jet1.Rapidity());
      h_raps->Fill(jet2.Rapidity());
      h_PT->Fill(PT_norm);
      h_QT->Fill(QT_norm);
      ...
      h2_phiQTPT_QT_mpi_24->Fill(QT_norm, angle12_orig);
