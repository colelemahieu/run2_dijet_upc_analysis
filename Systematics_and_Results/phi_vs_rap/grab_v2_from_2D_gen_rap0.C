// grab_v2_from_2D_gen_rap0.C
// This script takes in a 2D hist (phi vs rapidity) named h2_gen and uses a fit to extract the v2 value from each bin in rapidity. The output is a 5-bin 1D histogram
...
void grab_v2_from_2D_gen_rap0(string file_string)
{
  // Read File
  ...
  TH1F *h_v2_qt0 = new TH1F("h_v2_qt0","h_v2_qt0", nBins, edges);
  TH1F *h_v4_qt0 = new TH1F("h_v4_qt0","h_v4_qt0", nBins, edges);

  
  // loop through histograms
  //TH2D *h2_phi12_rap_qt = (TH2D*)file->Get("h2_genMatch");
  TH2D *h2_phi12_rap_qt = (TH2D*)file->Get("h2_gen");
  ...
  for (int rapBin=1; rapBin<6; rapBin++)
      ...
      for (int phiBin=1; phiBin<13; phiBin++)
	{
      ...
      h_v2_qt0->SetBinContent(rapBin, fit[rapBin-1]->GetParameter(1));
      h_v2_qt0->SetBinError(rapBin, fit[rapBin-1]->GetParError(1));
      h_v4_qt0->SetBinContent(rapBin, fit[rapBin-1]->GetParameter(2));
      h_v4_qt0->SetBinError(rapBin, fit[rapBin-1]->GetParError(2));
