// grab_v2_from_2D_gen.C (qt)
// This script takes in a 2D hist (phi vs QT) named h2_gen and uses a fit to extract the v2 value from each bin in QT. The output is a 5-bin 1D histogram 
...
void grab_v2_from_2D_gen(string file_string)
{
  // Read File
  TFile *file = new TFile(file_string.c_str());
  TH2D *h2_gen = (TH2D*)file->Get("h2_gen");
  TH2D *h2_genMatch = (TH2D*)file->Get("h2_genMatch");
  TH2D *h2_miss = (TH2D*)file->Get("h2_miss");
  ...
  for (int qtBin=1; qtBin<6; qtBin++)
	...
	{
	  h_phi[qtBin-1]->SetBinContent(phiBin, h2_gen->GetBinContent(qtBin, phiBin));
	  h_phi[qtBin-1]->SetBinError(phiBin, h2_gen->GetBinError(qtBin, phiBin));
