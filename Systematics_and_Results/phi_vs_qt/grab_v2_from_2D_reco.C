// grab_v2_from_2D_reco.C (qt)
// This script takes in a 2D hist (phi vs QT) named h2_reco and uses a fit to extract the v2 value from each bin in QT. The output is a 5-bin 1D histogram
...
void grab_v2_from_2D_reco(string file_string)
{
  // Read File
  TFile *file = new TFile(file_string.c_str());
  TH2D *h2_unf = (TH2D*)file->Get("h2_reco");
