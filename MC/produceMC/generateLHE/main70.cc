// this produces Pythia photonuclear LHE files
// ran with Pythia8p311
...
//#include "/afs/cern.ch/user/c/clemahie/Pythia8p311/pythia8311/include/Pythia8/Pythia.h"
...
int main(int argc, char* argv[]) {
  ...
  // Random Seed
  int jobIndex = std::stoi(argv[2]); 
  int seed = 1234 + jobIndex;
  ...
  // photon resolved and direct processes
  //pythia.readString("Photon:ProcessType = 1");

  // Limit partonic pThat.
  //pythia.readString("PhaseSpace:pTHatMin = 15.0");
  //pythia.readString("PhaseSpace:pTHatMin = 13.0");
  pythia.readString("PhaseSpace:pTHatMin = 12.5");
  //pythia.readString("PhaseSpace:pTHatMin = 11.0");
  //pythia.readString("PhaseSpace:pTHatMin = 10.0");

  //std::cout << "Pythia pthatmin=" << pythia.settings.parm("PhaseSpace:pTHatMin") << std::endl;

  // Use optimized pT0ref for photon-hadron.
  pythia.readString("MultipartonInteractions:pT0Ref = 3.0");
  //pythia.readString("PartonLevel:MPI = off");  // turn off MPI
  ...
  // Number of events.
  int nEvent = 11000; // 320000 (13000) 1000  20000



   // Create and open file for LHEF output
   ...
   //std::string outString = "/eos/cms/store/group/phys_heavyions/clemahie/MC_Official_Request_test/pythia_" + indexString + ".lhe";
   std::string outString = argv[1];
   myLHEF3.openLHEF(outString);
   myLHEF3.setInit();
   myLHEF3.initLHEF();
  ...
  for (int iEvent = 0; iEvent < nEvent; ++iEvent) {
    ...
    // Event pthat
    //std::cout << "Event " << iEvent << " pTHat = " << pythia.info.pTHat() << std::endl;
    // Event weight.
